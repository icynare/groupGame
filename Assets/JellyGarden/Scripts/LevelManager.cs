...
public class LevelManager : MonoBehaviour
{
        ...
        if (LevelManager.THIS.gameStatus == GameState.Playing)
        {
            if (Input.GetMouseButtonDown(0))
            {
                ...
                if (hit != null)
                {
                    ...
                    if (curTime < gapTime && touchCount == 2)
                    {
                        Debug.Log("小于gapTime：" + curTime);
                        touchCount = 0;
                     }
                    else
                    {
                    ...
                    curTime = 0;
                    if (oldHit == hit.GetHashCode())
                    {
                        if (touchCount == 0)
                        {
                            if(!LevelManager.THIS.DragBlocked)
                                item.doubleClick = true;
                            Debug.Log("点击相同的方块:" + oldHit);
                        }
                    }
                    else
                    {
                        oldHit = hit.GetHashCode();
                    }
    ...
    IEnumerator DestroyDoubleBombCor(int col)
    {
        ...
        Debug.Log("DestroyDoubleBombCor");
